...
public class AulaModificar extends javax.swing.JDialog {
    int id;
    GestorBD bd;
    
    public AulaModificar(int fila) {
        id = fila;
        int idAula = fila;
        initComponents();
    ...
    public void guardarBaseDatos() throws HeadlessException {
        ...
        try {           
            ...
            if (repetido){
                ...
                String sentencia = "UPDATE `icba`.`aula` SET `sigla` = '"+sigla+"',"
                        + "`capacidad` = '"+capacidad+"',"
                        + "`equipo` = '"+descripcion+"' "                        
                ...
                //if (registrosIni != registrosFin){
                    JOptionPane.showMessageDialog(null,"Los datos del aula fueron actualizados exitosamente","Actualización de Aula",JOptionPane.INFORMATION_MESSAGE) ;
                    bd.cerrar();
                    dispose();
                //}
               // else
