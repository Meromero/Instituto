...
public class HorarioBuscarEstado extends javax.swing.JDialog {
    MiModelo modelo;
    
    public HorarioBuscarEstado() {
        initComponents();
        buscarDia();
    }
    ...
    public void elegirHorario(){
        ...
        if (fila == -1)
            JOptionPane.showMessageDialog(null,"Error: Es necesario seleccionar un horario","Buscar Horario",JOptionPane.INFORMATION_MESSAGE) ;
        else{
            horario = Integer.parseInt(String.valueOf(jTableHorarios.getValueAt(fila,0)));
            estado = String.valueOf(jTableHorarios.getValueAt(fila,3));
            String buscar = String.valueOf(jComboBoxDia.getSelectedItem());

            GestorBD bd = new GestorBD(); 
            int idHorario = horario;
            try {
                bd.getConnection();
                if ("Inactivo".equals(estado))
                    sentencia = "UPDATE `icba`.`horario` SET `estado` = '1'"
                        + "WHERE `horario`.`idHorario` = "+idHorario+";";
                else
                    sentencia = "UPDATE `icba`.`horario` SET `estado` = '0'"
                        + "WHERE `horario`.`idHorario` = "+idHorario+";";
            ...
            if ("Inactivo".equals(estado))
                JOptionPane.showMessageDialog(null,"Horario habilitado exitosamente","Modificación de Estado",JOptionPane.INFORMATION_MESSAGE) ;
            else
                JOptionPane.showMessageDialog(null,"Horario inhabilitado exitosamente","Modificación de Estado",JOptionPane.INFORMATION_MESSAGE) ;
            buscarHorarios(buscar);
        }
    }
    ...
    public void buscarHorarios(String buscar){
        ...
        try {
            rs = bd.consulta("Select * From `icba`.`horario` "
                    + "Where dia = '"+buscar+"' Order By horaInicio, horaFinal"); 
        } catch (SQLException ex) {
            Logger.getLogger(getName()).log(Level.SEVERE, null, ex);
        }
